...
import {
  extractIngredientMentionsFromText,
...
type IngredientReviewState = {
  ...
  userNoteIngredients?: string[];
  photoMealGuess?: string;
};
...
const copy = {
  en: {
    ...
    userNotesPriorityMessage: "I'll use your meal description and confirmed ingredients for the estimate.",
    ...
    confidenceLevels: {
      ...
      higher: 'Higher confidence: user notes or confirmed ingredients + context',
    },
    ...
    insightFallbacks: {
      onionBloating: 'High onion load may be associated with bloating',
      garlicBloating: 'Garlic may add a possible high-FODMAP bloating pattern',
      spicyBloating: 'Spice load may be associated with bloating or cramps',
      creamyReflux: 'Creamy sauce may be associated with reflux',
      greasyReflux: 'Greasy fat load may be associated with reflux',
      acidicReflux: 'Acidic ingredients may be associated with reflux discomfort',
  ...
  de: {
    ...
    userNotesPriorityMessage: 'Ich verwende deine Mahlzeitenbeschreibung und die bestätigten Zutaten für die Einschätzung.',
    lowerConfidenceNotice:
    ...
    confidenceLevels: {
      ...
      higher: 'Höhere Sicherheit: Nutzerangaben oder bestätigte Zutaten + Kontext',
    },
    lowEstimateGentleNote:
    ...
    insightFallbacks: {
      onionBloating: 'Hohe Zwiebelmenge kann mit Blähungen zusammenhängen',
      garlicBloating: 'Knoblauch kann zu einem möglichen FODMAP-Muster beitragen',
      spicyBloating: 'Schärfe kann mit Blähungen oder Krämpfen zusammenhängen',
      creamyReflux: 'Cremige Sauce kann mit Reflux zusammenhängen',
      greasyReflux: 'Fettige Speisen können mit Reflux zusammenhängen',
      acidicReflux: 'Säurehaltige Zutaten können mit Reflux-Beschwerden zusammenhängen',
      constipationFiber: 'Ballaststoffreiche Zutaten können Regelmäßigkeitsmuster unterstützen',
      constipationLowFiber: 'Geringe Ballaststoffdichte kann den Darmrhythmus bremsen',
      lowFiberDiversity: 'Wenig Ballaststoffvielfalt kann Darmwohlbefinden begrenzen',
      processedFat: 'Verarbeitete fettreiche Kombination kann weniger gut verträglich sein',
      balancedDiversity: 'Ausgewogenes Protein und gegartes Gemüse können Stabilität unterstützen',
      fodmapIbs: 'Hohe FODMAP-Belastung kann bei Reizdarm-Mustern weniger gut verträglich sein',
      fructan: 'Mögliches Fruktan-Muster notiert',
  ...
  fa: {
    ...
    userNotesPriorityMessage: 'برای برآورد، توضیح وعده و مواد تأییدشده شما را مبنا می‌گیرم.',
    lowerConfidenceNotice:
    ...
    confidenceLevels: {
      ...
      higher: 'اطمینان بالاتر: توضیح شما یا مواد تأییدشده همراه با زمینه',
    },
    lowEstimateGentleNote:
    ...
    instantReliefText: 'چای نعناع، کمپرس گرم روی شکم، تنفس آرام، آب کافی و استراحت را امتحان کنید. فعلاً غذایی را که به آن مشکوک هستید کنار بگذارید. اگر درد شدید، رو به بدتر شدن یا غیرمعمول است، کمک پزشکی بگیرید.',
    ...
    insightFallbacks: {
      onionBloating: 'بار بالای پیاز ممکن است با نفخ همراه باشد',
      garlicBloating: 'سیر می‌تواند به یک الگوی احتمالی FODMAP مربوط باشد',
      spicyBloating: 'غذای تند ممکن است با نفخ یا گرفتگی همراه باشد',
      creamyReflux: 'سس خامه‌ای ممکن است با ریفلاکس همراه باشد',
      greasyReflux: 'چربی زیاد ممکن است با ریفلاکس همراه باشد',
      acidicReflux: 'مواد اسیدی ممکن است با ناراحتی ریفلاکس همراه باشند',
      constipationFiber: 'مواد پرفیبر ممکن است از الگوی دفع منظم پشتیبانی کنند',
      constipationLowFiber: 'فیبر کم ممکن است ریتم روده را کند کند',
      lowFiberDiversity: 'تنوع کم فیبر ممکن است حمایت از تندرستی گوارش را محدود کند',
      processedFat: 'ترکیب پرچرب و فرآوری‌شده ممکن است برای گوارش کمتر ملایم باشد',
      balancedDiversity: 'پروتئین متعادل و سبزی پخته ممکن است از ثبات گوارش پشتیبانی کند',
      fodmapIbs: 'بار بالای FODMAP ممکن است برای الگوهای IBS کمتر سازگار باشد',
      fructan: 'الگوی احتمالی فروکتان بالا دیده شد',
...
function getIngredientConfidenceLevel(
  ...
  if (review.status === 'confirmed') return 'higher';
  return 'medium';
}
...
function getIngredientReviewFromUserNotes(
  text: string,
  language: AppLanguage,
  ...
  const dishSuggestion = findCommonDishIngredientSuggestion(trimmed, language);
  const noteIngredients = filterIngredientsForLanguage(
  ...
  return {
    ...
    userNoteIngredients: noteIngredients,
  };
}
...
export default function PhotoAnalysisScreen() {
  ...
  const [ingredientFeedbackKind, setIngredientFeedbackKind] = useState<IngredientFeedbackKind>(null);
  ...
  const ingredientEditFeedbackMessage =
    ingredientFeedbackKind === 'updated'
      ? t.ingredientUpdatedMessage
      : ingredientFeedbackKind === 'helpful'
        ? t.ingredientEditHelpfulMessage
        : '';
  ...
  useEffect(() => {
    ...
    getPhotoAnalysisHistory(user?.id)
      .then((history) => {
        ...
        setCustomIngredient('');
        setIngredientFeedbackKind(null);
        setPlanBMessage('');
        setUserFeedback([]);
        setWizardStep(4);
  ...
  const handlePrepareIngredientReview = async () => {
    ...
    const notesSuggestion = getIngredientReviewFromUserNotes(mealDescriptionText, language, t.photoMealDefault);
    if (notesSuggestion) {
      setIngredientReview(notesSuggestion);
      setWizardStep(3);
      return;
    }
            ...
            {wizardStep === 3 ? (
              <>
                ...
                <View style={styles.ingredientReviewCard}>
                  ...
                  {ingredientReview?.userNotesPrioritized ? (
                    <View style={[styles.ingredientNoticeRow, isRtlLanguage && styles.rtlRow]}>
                      <Ionicons name="checkmark-circle-outline" size={16} color="#B7F7D6" />
                      <Text style={[styles.ingredientNoticeText, isRtlLanguage && styles.rtlText]}>
                        {t.userNotesPriorityMessage}
                      </Text>
                    </View>
                  ) : null}

                  {ingredientEditFeedbackMessage ? (
                    <View style={[styles.ingredientNoticeRow, isRtlLanguage && styles.rtlRow]}>
                      <Ionicons name="create-outline" size={16} color="#B7F7D6" />
                      <Text style={[styles.ingredientNoticeText, isRtlLanguage && styles.rtlText]}>
                        {ingredientEditFeedbackMessage} {t.ingredientConfirmedUsageMessage}
                      </Text>
                    </View>
                  ) : null}
