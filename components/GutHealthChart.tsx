...
export function GutHealthChart({ userId }: { userId?: string }) {
  ...
  const points = useMemo<TrendPoint[]>(() => {
    ...
    return chronologicalRows.map((row) => ({
      id: String(row.id),
      label: useTimeLabels ? formatTimeLabel(row.created_at) : formatDateLabel(row.created_at),
  ...
  return (
    <View style={styles.card}>
      <View style={styles.header}>
        <View>
          <Text style={styles.kicker}>Date / Score</Text>
          <Text style={styles.title}>Gut Health Trend</Text>
        </View>
        <View style={styles.pill}>
          <Text style={styles.pillText}>Last 7 logs</Text>
        </View>
      </View>
      ...
      {isLoading ? (
        ...
        <View style={styles.placeholder}>
          <Text style={styles.placeholderText}>Log more meals to see your trend chart!</Text>
        </View>
      ) : (
        <>
          ...
          <Text style={styles.caption}>Success green line tracks meal impact score from your latest analyses.</Text>
        </>
      )}
    </View>
  );
