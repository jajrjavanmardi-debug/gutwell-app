...
if (!supabaseUrl || !supabaseKey) {
  throw new Error(
    '[NutriFlow] Missing Supabase config. Copy .env.example to .env, set EXPO_PUBLIC_SUPABASE_URL and EXPO_PUBLIC_SUPABASE_ANON_KEY (anon key from Supabase -> Project Settings -> API), then restart Expo.'
  );
}
...
function assertRealSupabaseCredentials(url: string, key: string): void {
  ...
  if (isCloud && !key.startsWith('eyJ')) {
    throw new Error(
      '[NutriFlow] EXPO_PUBLIC_SUPABASE_ANON_KEY should be the anon public key (long JWT starting with eyJ) from Project Settings -> API, not the service_role key.'
    );
  }
