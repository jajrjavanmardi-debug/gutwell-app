...
export async function analyzeMealPhotoWithGroq(
  ...
  const adviceTail = [
    ...
    'Dynamic estimate rule: never reuse a default estimate. Use the full 1-10 range. Estimate 8-10 for balanced meals with gentle protein, cooked plants, fiber diversity, or constipation-supportive fiber. Estimate 5-7 only for mixed meals. Estimate 1-4 for possible trigger-heavy patterns such as fast food, greasy/fried foods, creamy sauces, spicy foods, lactose-heavy meals, high-FODMAP onion/garlic/beans/lentils/wheat, wheat/barley for celiac, or reflux-associated tomato/citrus/coffee/chocolate/alcohol.',
...
export async function reviseMealAnalysisWithGroq(
  ...
  const prompt = [
    ...
    `- Recalculate the ${languageCopy.mealImpactScore} from the corrected meal, current symptoms, and known conditions. Use the full 1-10 range: 8-10 for gentle balanced meals that may fit the current context, 5-7 for mixed meals, 1-4 for strong possible-trigger combinations. Never reuse a default 4/10 or 6/10 estimate unless the corrected meal truly fits that range.`,
    ...
    `- Return Markdown only. Include a compact Summary Table for "${languageCopy.mealImpact}" with columns "${languageCopy.meal}", "${languageCopy.symptoms}", "${languageCopy.mealImpactScore}", "${languageCopy.why}", one "${languageCopy.gutScore}: [####------] X/10" progress bar using the recalculated estimate, "${languageCopy.analysisInsight}:" with 2-3 concise estimate-rationale bullets, and max 3 "${languageCopy.tips}" bullet points.`,
