...
const MEAL_LANGUAGE_COPY: Record<AppLanguage, {
  ...
  en: {
    ...
    mealImpact: 'Meal Fit Estimate',
    meal: 'Meal',
    symptoms: 'Symptoms',
    mealImpactScore: 'Educational Meal Estimate',
    why: 'Why',
    gutScore: 'Pattern-based estimate',
    analysisInsight: 'Analysis Insight',
    tips: 'Tips',
    instantRelief: 'Comfort Support',
    footer: 'This is an educational estimate based on your profile and meal context, not a diagnosis. Look for patterns over time.',
  },
  de: {
    languageName: 'German (Deutsch)',
    languageRule: 'Write the entire answer strictly in German.',
    mealImpact: 'Einschätzung zur Mahlzeiten-Passung',
    meal: 'Mahlzeit',
    symptoms: 'Symptome',
    mealImpactScore: 'Orientierende Mahlzeiten-Einschätzung',
    why: 'Warum',
    gutScore: 'Musterbasierte Einschätzung',
    analysisInsight: 'Analyse-Einblick',
    tips: 'Tipps',
    instantRelief: 'Sanfte Hilfe',
    footer: 'Dies ist eine orientierende Einschätzung auf Basis deines Profils und Mahlzeitenkontexts, keine Diagnose. Achte über Zeit auf Muster.',
  },
  fa: {
    ...
    mealImpact: 'برآورد تناسب وعده',
    meal: 'غذا',
    symptoms: 'علائم',
    mealImpactScore: 'برآورد آموزشی وعده غذایی',
    why: 'چرا',
    gutScore: 'برآورد مبتنی بر الگو',
    analysisInsight: 'بینش تحلیل',
    tips: 'نکات',
    instantRelief: 'راهنمای آرام‌سازی',
    footer: 'این یک برآورد آموزشی بر اساس پروفایل و زمینه وعده غذایی شماست، نه تشخیص. الگوها را در طول زمان دنبال کنید.',
  },
};
...
const COACH_LANGUAGE_COPY: Record<AppLanguage, {
  ...
  de: {
    ...
    symptomCheck: 'Symptom-Check',
    coachTip: 'Coach-Tipp',
    footer: 'Orientierender Einblick, keine Diagnose und keine medizinische Beratung. Beobachte Muster über die Zeit und suche Hilfe bei starken oder ungewöhnlichen Symptomen.',
  },
  fa: {
    languageName: 'Persian (فارسی)',
  ...
  const prompt = [
    ...
    hasSpecificFood
      ...
      : [
          ...
          'Do not analyze "this meal", do not say the person ate any food, and do not provide a meal fit estimate.',
    ...
    'If a wellness estimate is present, frame the advice as one small pattern-tracking step that may support a steadier estimate over time.',
...
export async function analyzeMealPhotoWithGroq(
  ...
  const profileHead = [
    `Current wellness estimate baseline: ${gutScoreSummary}.`,
    `Known wellness profile context: ${conditionSummary}.`,
    'If profile context is "General Gut Wellness", provide balanced digestive-wellness guidance without assuming IBS, celiac, lactose intolerance, or other specific conditions.',
  ...
  const adviceTail = [
    ...
    `Provide a neutral meal reflection for this specific meal. Do not rate the meal. Do not output a numeric score. Do not output X/10. Do not classify food as good or bad.`,
    'Confidence rule: instead of a numeric score, indicate confidence in the reflection as text only: Low confidence, Moderate confidence, or Higher confidence. Base confidence on how clearly the meal is visible and how well the ingredients match the current symptom context.',
    'Symptom interaction rule: bloating plus onion, garlic, wheat, beans, lentils, carbonated drinks, or spicy foods must lower the estimate strongly. Reflux plus greasy, spicy, creamy, acidic, coffee, chocolate, or alcohol must lower the estimate strongly. Constipation plus fiber-rich foods such as oats, chia, flax, kiwi, prunes, berries, legumes, cooked vegetables, or soup should raise the estimate unless bloating/IBS comfort concerns dominate.',
    ...
    'Identify likely foods visible in the image when helpful, estimate how the meal may fit the current profile and symptoms, and explain the main reasons in friendly practical language.',
    ...
    `Output format requirement: return Markdown only. First include a compact Summary Table for "${languageCopy.mealImpact}" with columns "${languageCopy.meal}", "${languageCopy.symptoms}", "${languageCopy.why}". Do not include a score column. Do not include a progress bar. Do not output X/10. Then include "${languageCopy.analysisInsight}:" with 2-3 concise bullet points about ingredients and symptom context. Then include "${languageCopy.tips}:" with max 3 bullet points.`,
...
export async function reviseMealAnalysisWithGroq(
  ...
  const prompt = [
    ...
    '- Provide a neutral reflection on the corrected meal. Do not recalculate a numeric score. Do not output X/10. Indicate confidence as text only: Low confidence, Moderate confidence, or Higher confidence.',
    ...
    `- Return Markdown only. Include a compact Summary Table for "${languageCopy.mealImpact}" with columns "${languageCopy.meal}", "${languageCopy.symptoms}", "${languageCopy.why}". Do not include a score column. Do not include a progress bar. Do not output X/10. Include "${languageCopy.analysisInsight}:" with 2-3 concise bullets about ingredients and symptom context, and max 3 "${languageCopy.tips}" bullet points.`,
