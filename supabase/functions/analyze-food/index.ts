...
function buildMealTextOnlyPrompt(body: MealTextBody & { mealDescription?: string }): string {
  ...
  return [
    ...
    "- If the person has reported bloating, gas or abdominal discomfort — in their symptoms, their notes, or their current state — you may name specific ingredients in THIS meal that may contribute to bloating, gas or digestive discomfort for some people (for example large portions of beans, onion, garlic, wheat, or sugar alcohols), and offer one simple alternative they could try and compare against their own response next time. Frame it as a personal experiment, never as a rule, never as a list of foods to avoid, and never as a diet plan. Do not default to peppermint or ginger tea unless the user explicitly reports pain, nausea, gas, or bloating in their notes.",
    "- A condition label such as IBS is context for relevance and tone only: it never licenses an elimination diet, a named clinical programme, or an instruction to remove a food group. Where a meaningful dietary change seems relevant, say that a doctor or registered dietitian is the right person to plan it. Do not name the condition in your reply, and never give it as the reason for the score, for calling a food a problem, for a suggestion, or for how the person feels — write about this meal and the symptoms they reported instead. Describe ingredients in plain language. Do not use named clinical diet or protocol terminology in the reply.",
...
function buildMealRevisePrompt(body: MealReviseBody): { persona: string; prompt: string } {
  ...
  const prompt = [
    ...
    "- A condition label such as IBS is context for relevance and tone only. Do not name the condition in your revised reply, and never give it as the reason for the score, for calling a food a problem, or for a suggestion. Describe ingredients in plain language. Do not use named clinical diet or protocol terminology in the reply. Prefer may, might, possible, or can for some people. Do not state a causal claim with words such as likely, caused, or triggered.",
    "",
    ...FIVE_SECTION_FORMAT_RULES,
    "",
    ...fiveSectionStructure({
